...
{
    int origin = (width * height) - width;
    return depthPixels[origin + x - (y * width)];
}
...
{
    int origin = (width * height) - width;
    depthPixels[origin + x - (y * width)] = depth;
}
...
void Raster::drawTriangle3D_Barycentric(Triangle3D t)
{
    if (t.shouldDraw)
    {
        ...
        float minx = fmin(t2.v1.x, t2.v2.x);
        minx = fmin(minx, t2.v3.x);
        float maxx = fmax(t2.v1.x, t2.v2.x);
        maxx = fmax(maxx, t2.v3.x);
        float miny = fmin(t2.v1.y, t2.v2.y);
        miny = fmin(miny, t2.v3.y);
        float maxy = fmax(t2.v1.y, t2.v2.y);
        maxy = fmax(maxy, t2.v3.y);
        float lambda1;
        float lambda2;
        float lambda3;
        ...
        for (int x = minx; x <= maxx; x++)
        {
            for (int y = miny; y <= maxy; y++)
            {
                t2.calculateBarycentricCoordinates(Vector2(x, y), lambda1, lambda2, lambda3);
                if (lambda1 >= 0 && lambda2 >= 0 && lambda3 >= 0 && x >= 0 && x < width && y >= 0 && y < width)
                {
                    // Color mycolor(lambda1,lambda2,lambda3);
                    Color myColor = (t2.c1 * lambda1) + (t2.c2 * lambda2) + (t2.c3 * lambda3);
                    float myDepth = (t.v1.z * lambda1 + t.v2.z * lambda2 + t.v3.z * lambda3);

                    if (myDepth < getDepthPixel(x, y))
                    {
                        setDepthPixel(x, y, myDepth);
